...
void InterruptManager::register_ISR(const uint32_t pin, const ISRFuncPtr ISR, const uint32_t signal_type, const ISR_Type run_type)
{
	...
	if(int_settings[i].pin == pin || int_settings[i].pin == -1){
		...
		LMark;
		int_settings[i] = {(int)pin, ISR, signal_type, run_type, (ISR) ? true : false };
	}
 	else{
		print_module_label();
...
bool InterruptManager::reconnect_interrupt(const uint32_t pin)
{
  ...
  LMark;
	...
	// If ISR provided
	if (settings.ISRFunc != nullptr) {
    LMark;
		ISRFuncPtr tmpISR = (settings.run_type == ISR_Type::IMMEDIATE) ? settings.ISRFunc : default_ISRs[i];
...
void InterruptManager::run_ISR_bottom_halves()
{
	if (interrupts_enabled) {
      ...
      LMark;
			...
			if ( (interrupt_triggered[i]) && (int_settings[i].ISRFunc != nullptr) ) {
        LMark;

				// LPrintln("I: ", i);

				// Run bottom half ISR
				int_settings[i].ISRFunc();
...
void InterruptManager::interrupt_reset(const uint32_t pin)
{
	...
	if (int_settings[i].pin == pin && int_settings[i].ISRFunc != nullptr) {
		// Attach interrupt with specified type
		attachInterrupt(digitalPinToInterrupt(pin), int_settings[i].ISRFunc, (int_settings[i].type<5) ? int_settings[i].type : 0 );
	}
	else {
		print_module_label();
		LPrintln("Error: Pin ", pin, " has not been registered yet");
...
///////////////////////////////////////////////////////////////////////////////
void InterruptManager::register_internal_timer(const unsigned int duration, const ISRFuncPtr ISR, const bool repeat, const ISR_Type run_type)
{
	internal_timer.ISRFunc		= ISR;
...
bool InterruptManager::run_pending_internal_timer_ISR()
{
  LMark;
	if ( (internal_timer.run_type == ISR_Type::CHECK_FLAG) &&
		 (internal_timer.ISRFunc != nullptr) &&
		 (rtcCounter.getFlag()) ) {
		rtcCounter.clearFlag();
		LPrintln("Executing ISR");
    LMark;
		internal_timer.ISRFunc();
		return true;
	}
	return false;
}
