...
private:
	...
	struct IntDetails {
		int	pin;
		ISRFuncPtr	ISRFunc;			///< Function pointer to ISR. Set null if no interrupt linked
	...
	struct InternalTimerDetails {
		ISRFuncPtr	ISRFunc;			///< Function pointer to ISR. Set null if no interrupt linked
		ISR_Type	run_type;		///< True if ISR is called directly upon interrupt, false if called next check of flags
		unsigned int		duration;		///< The timer duration
	...
	/// Contains information defining a timer's configuration
	struct TimerDetails {
		ISRFuncPtr	ISRFunc;			///< Not a real ISR, just a function called if timer has expired
		unsigned int		duration;		///< The timer duration
		bool		repeat;			///< Whether or not timer should repeat
		bool		enabled;		///< Whether or not this timer is enabled
	};
...
/*@{*/ //======================================================================
	...
	void		register_ISR(const uint32_t pin, ISRFuncPtr ISRFunc, const uint32_t signal_type, ISR_Type run_type);
...
// maybe remove in favor of internal timers
	...
	void		register_timer(const uint8_t timer_num, const unsigned long duration, const ISRFuncPtr ISRFunc, const bool repeat);
...
/*@{*/ //======================================================================
	...
	/// @param[in]	repeat			Whether or not to be a repeating alarm
	/// @param[in]	run_type	Whether the interrupt runs immediately, else sets flag to check and runs ISR when flag checked
	void		register_internal_timer(const unsigned int duration, const ISRFuncPtr ISRFunc, const bool repeat, const ISR_Type run_type);
